...
Usage:
    python3 anpr_full_easyocr.py                  # live camera
    python3 anpr_full_easyocr.py --test           # test on static images
    python3 anpr_full_easyocr.py --benchmark      # benchmark speed
    python3 anpr_full_easyocr.py --gstreamer      # use CSI Pi Camera via GStreamer
...
# =============================================================================
# OCR — EasyOCR (more accurate, slower)
# =============================================================================
def ocr_plate(plate_img, reader):
    """
...
def main():
    ...
    # OCR setup (EasyOCR — more accurate but slower)
    ...
    if not cap.isOpened():
        ...
        print("  Try: python3 anpr_full_easyocr.py --video plate_video.mp4")
        print("  Or:  python3 anpr_full_easyocr.py --gstreamer")
        print("  Or:  python3 anpr_full_easyocr.py --camera 1")
        sys.exit(1)

    if not args.gstreamer:  # cap.set() disrupts GStreamer CSI pipeline
    ...
    for plate in unique_plates:
        # Get latest status for this plate
        for l in logs:
            if l[0] == plate:
